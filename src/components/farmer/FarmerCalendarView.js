...
export default function FarmerCalendarView({
    ...
    // Lấy công việc cho slot thời gian cụ thể - cải tiến để hiển thị nhiều tasks cùng lúc
    const getTasksForTimeSlot = (date, hour) => {
        ...
        return dayTasks.filter((task) => {
            ...
            if (task.thoi_gian_bat_dau && task.thoi_gian_ket_thuc) {
                ...
                // Task hiển thị nếu nó bắt đầu trong slot này hoặc đang diễn ra trong slot này
                return (taskStartHour === hour) || (hour >= taskStartHour && hour < taskEndHour);
    ...
    return ( <
        ...
        sx = {
            { display: 'flex', height: '100vh', bgcolor: '#f5f5f5' } } > { /* Sidebar bên trái */ } <
        ...
        Typography variant = "body2"
        fontWeight = "bold" > { weekDates.reduce((sum, date) => sum + getDayStats(date).total, 0) } <
        /Typography> <
        /Box> <
        Box sx = {
            { display: 'flex', justifyContent: 'space-between' } } >
        <
        Typography variant = "body2"
        color = "success.main" > Hoàn thành: < /Typography> <
        Typography variant = "body2"
        fontWeight = "bold"
        color = "success.main" > { weekDates.reduce((sum, date) => sum + getDayStats(date).completed, 0) } <
        /Typography> <
        /Box> <
        Box sx = {
            { display: 'flex', justifyContent: 'space-between' } } >
        <
        Typography variant = "body2"
        color = "warning.main" > Đang làm: < /Typography> <
        Typography variant = "body2"
        fontWeight = "bold"
        color = "warning.main" > { weekDates.reduce((sum, date) => sum + getDayStats(date).inProgress, 0) } <
        /Typography> <
        /Box> <
        /Box> <
        /Box>
                    ...
                    /Typography> {
                        stats.total > 0 && ( <
                            Box sx = {
                                ...
                                stats.completed > 0 && ( <
                                    Box sx = {
                                        { width: 4, height: 4, borderRadius: '50%', bgcolor: '#4caf50' } }
                                    />
                                )
                            } {
                                stats.inProgress > 0 && ( <
                                    Box sx = {
                                        { width: 4, height: 4, borderRadius: '50%', bgcolor: '#ff9800' } }
                                    />
                                )
                            } {
                                stats.pending > 0 && ( <
                                    Box sx = {
                                        { width: 4, height: 4, borderRadius: '50%', bgcolor: '#9e9e9e' } }
                                    />
                                )
                            } <
                            /Box>
        ...
        <
        Button onClick = {
            () => setOpenViewDialog(false) } > Đóng < /Button> {
        ...
        <
        Typography variant = "h6"
        gutterBottom > { selectedTask ? .ten_cong_viec } <
        /Typography>
        ...
        Select value = { updateForm.trang_thai }
        onChange = {
            (e) => setUpdateForm({...updateForm, trang_thai: e.target.value }) }
        label = "Trạng thái" >
        {
            statuses.map((status) => ( <
        ...
        <
        TextField fullWidth label = "Kết quả"
        multiline rows = { 3 }
        value = { updateForm.ket_qua }
        onChange = {
            (e) => setUpdateForm({...updateForm, ket_qua: e.target.value }) }
        sx = {
            { mb: 2 } }
        ...
        onChange = {
            (e) => setUpdateForm({...updateForm, ghi_chu: e.target.value }) }
