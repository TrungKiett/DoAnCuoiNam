...
// lấy mã lô trồng
  ...
  const handleSubmit = async () => {
    if (!form.noi_dung) { alert("Vui lòng nhập nội dung!"); return; }
    if (!form.ma_nong_dan) { alert("Không tìm thấy mã nông dân!"); return; }

    try {
      const formData = new FormData();
      ["noi_dung", "loai_van_de", "ngay_bao_cao", "ma_lo_trong", "ma_nong_dan", "trang_thai", "ghi_chu"].forEach(key =>
        formData.append(key, form[key] || "")
      );
      if (form.file) formData.append("hinh_anh", form.file);

      const res = await fetch("http://localhost/doancuoinam/src/be_management/acotor/farmer/de_xuat_xu_li_ki_thuat.php", {
        method: "POST",
        body: formData,
        credentials: "include"
      });
      const data = await res.json();
      if (data.success) { alert("Đã lưu thành công!"); setOpen(false); }
    ...
    <Box className="calendar-weekly-container" sx={{ display: 'flex', height: '100vh', bgcolor: '#f5f5f5' }}>
      ...
      <Dialog open={openViewDialog} onClose={() => setOpenViewDialog(false)} maxWidth="md" fullWidth>
        ...
        <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
          ...
          <Button onClick={() => setOpen(true)} variant="contained" color="primary">Đề xuất kĩ thuật</Button>
          <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md">
            <DialogTitle>Đề xuất kĩ thuật</DialogTitle>
            <DialogContent dividers>
              <TextField margin="dense" label="Nội dung" name="noi_dung" value={form.noi_dung} onChange={handleChange} fullWidth multiline rows={3} />
              <TextField margin="dense" label="Loại vấn đề" name="loai_van_de" value={form.loai_van_de} onChange={handleChange} fullWidth select>
              ...
              <TextField margin="dense" label="Ngày báo cáo" type="date" name="ngay_bao_cao" value={form.ngay_bao_cao} onChange={handleChange} fullWidth InputLabelProps={{ shrink: true }} />
              <TextField margin="dense" label="Mã nông dân" name="ma_nong_dan"   value={form.ma_nong_dan} fullWidth InputProps={{ readOnly: true }} />
               <TextField margin="dense" label="Mã lô trồng" name="ma_lo_trong" value={form.ma_lo_trong} fullWidth InputProps={{ readOnly: true }} />
...
{/* ../../be_management/uploads/ */}
            ...
            <DialogActions>
              ...
              <Button onClick={handleSubmit} variant="contained" color="primary">Gửi</Button>
            </DialogActions>
          </Dialog>
        </Dialog>
        <DialogActions>
