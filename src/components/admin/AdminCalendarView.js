...
export default function AdminCalendarView({ tasks = [], farmers = [], plans = [], onCreateTask, onUpdateTask, onDeleteRange }) {
    ...
    return ( <
            Box sx = {
                { display: 'flex', height: '100vh', bgcolor: '#f5f5f5' } } > { /* Sidebar */ } <
            Paper sx = {
            ...
            sx = {
                { fontWeight: 'bold', mb: 1 } } > Lịch làm việc < /Typography> <
            Button startIcon = { < AddIcon / > }
            variant = "contained"
            size = "small"
            onClick = {
                () => openCreateFor(new Date()) } > Thêm lịch làm việc < /Button> <
            /Box>
            ...
            sx = {
                { mb: 1, fontWeight: 'bold' } } > { currentDate.toLocaleDateString('vi-VN', { month: 'long', year: 'numeric' }) } < /Typography> <
            Box sx = {
                ...
                ['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'].map(d => ( <
                    ...
                    variant = "caption"
                    sx = {
                        { textAlign: 'center', p: 0.5 } } > { d } < /Typography>
                ))
            } {
                weekDays.map((d, idx) => ( <
                    Box key = { idx }
            ...
            Typography variant = "subtitle2"
            sx = {
                { mb: 1, fontWeight: 'bold' } } > Loại công việc < /Typography> <
            List dense > {
                taskTypes.map((t) => ( <
            ...
            IconButton onClick = {
                () => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate() - 7)) } > < ChevronLeftIcon / > < /IconButton> <
            Typography variant = "h6"
            sx = {
                { minWidth: 220, textAlign: 'center' } } > { weekDays[0].toLocaleDateString('vi-VN', { day: 'numeric', month: 'long' }) } - { weekDays[6].toLocaleDateString('vi-VN', { day: 'numeric', month: 'long', year: 'numeric' }) } <
            /Typography> <
            IconButton onClick = {
                () => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate() + 7)) } > < ChevronRightIcon / > < /IconButton> <
            /Box> <
            Box sx = {
            ...
            { /* Days */ } {
                weekDays.map((date, idx) => ( <
                    Box key = { idx }
                    sx = {
                        { flex: 1, borderRight: idx < 6 ? '1px solid #e0e0e0' : 'none' } } > { /* Header */ } <
                    Box sx = {
                    ...
                    sx = {
                        { fontWeight: date.toDateString() === new Date().toDateString() ? 'bold' : 'normal' } } > { formatHeader(date) } < /Typography> <
                    /Box>
                    ...
                    onChange = {
                        ...
                        statuses.map(s => < MenuItem key = { s.value }
                            ...
                            onChange = {
                                ...
                                farmers.map(f => < MenuItem key = { f.id }
                                    value = { String(f.id) } > { f.full_name || `ID ${f.id}` } < /MenuItem>)} <
                                    /Select> <
                                    /FormControl> <
                                    /DialogContent> <
                                    DialogActions >
                                    <
                                    Button onClick = {
                                        () => setOpenCreate(false) } > Hủy < /Button> <
                                    Button variant = "contained"
                                    startIcon = { < AddIcon / > }
                                    onClick = { async() => { try { await onCreateTask({...form });
                                                setSnackbar({ open: true, message: 'Tạo công việc thành công!', severity: 'success' });
                                                setOpenCreate(false); } catch (e) { setSnackbar({ open: true, message: e.message, severity: 'error' }); } } } > Tạo mới < /Button> <
                                    /DialogActions> <
                                    /Dialog>
                                    ...
                                    DialogContent sx = {
                                        ...
                                        viewingTask ? ( <
                                            ...
                                            Typography variant = "h6"
                                            sx = {
                                                { fontWeight: 700 } } > { viewingTask.ten_cong_viec } < /Typography> <
                                            ...
                                            onChange = {
                                                ...
                                                statuses.map(s => < MenuItem key = { s.value }
                                                    ...
                                                    sx = {
                                                        { width: '100%' } } > { snackbar.message } < /Alert> <
                                                    /Snackbar> <
                                                    /Box>
                                                );
                                            }