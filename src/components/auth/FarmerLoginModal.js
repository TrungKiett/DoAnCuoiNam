...
import { Dialog, DialogTitle, DialogContent, DialogActions, TextField, Button, Box, Typography, Alert, CircularProgress, IconButton } from '@mui/material';
import { Close as CloseIcon, Person as PersonIcon, Lock as LockIcon, Phone as PhoneIcon } from '@mui/icons-material';
...
const FarmerLoginModal = ({ open, onClose, onLoginSuccess }) => {
  ...
  const [vaiTro, setVaiTro] = useState('nong_dan');
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const response = await fetch(`${base}${root}/src/be_management/api/simple_login.php`, {
        method: 'POST',
  ...
  const handleSendOtp = async () => {
    try {
      setLoading(true);
      setError('');
      const res = await fetch(`${base}${root}/src/be_management/controller/components/auth/forgot.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: forgotValue, vai_tro: vaiTro })
      });
      const data = await res.json();
      if (data.status === 'success') setMode('verify');
  ...
  const handleVerifyOtpAndReset = async () => {
    try {
      setLoading(true);
      setError('');
      const res = await fetch(`${base}${root}/src/be_management/controller/components/auth/reset_password.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ otp, email: forgotValue, vai_tro: vaiTro, new_password: newPassword })
      });
      const data = await res.json();
      if (data.status === 'success') setMode('reset_success');
      else setError(data.message || 'OTP không hợp lệ');
    } catch (err) {
      ...
      <form onSubmit={mode === 'login' ? handleSubmit : (e) => { e.preventDefault(); }}>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            {mode === 'reset_success' && (<Alert severity="success">Đổi mật khẩu thành công! Vui lòng đăng nhập lại.</Alert>)}
          </Box>
        </DialogContent>
        ...
        <DialogActions sx={{ p: 2, gap: 1 }}>
          ...
          {mode === 'login' ? (
            <Button type="submit" variant="contained" disabled={loading} startIcon={loading ? <CircularProgress size={20} /> : <PersonIcon />}>
