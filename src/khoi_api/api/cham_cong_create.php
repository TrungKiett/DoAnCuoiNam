...
$ma_nguoi_dung_list = $input['ma_nguoi_dung'] ?? []; // Mảng các ID người dùng hoặc chuỗi CSV
...
// Chấp nhận chuỗi CSV hoặc giá trị đơn lẻ, chuyển thành mảng
if (!is_array($ma_nguoi_dung_list)) {
    if (is_string($ma_nguoi_dung_list) && trim($ma_nguoi_dung_list) !== '') {
        $ma_nguoi_dung_list = array_map('trim', explode(',', $ma_nguoi_dung_list));
    } elseif ($ma_nguoi_dung_list !== null && $ma_nguoi_dung_list !== '') {
        $ma_nguoi_dung_list = [ $ma_nguoi_dung_list ];
    } else {
        $ma_nguoi_dung_list = [];
    }
}

if (empty($ma_nguoi_dung_list)) {
    http_response_code(400);
    echo json_encode(["success" => false, "error" => "ma_nguoi_dung phải là mảng (hoặc chuỗi CSV) không rỗng"]);
    exit;
}
