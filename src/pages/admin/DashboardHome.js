      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {statCards.map((card) => (
          <Grid item xs={12} sm={6} md={3} key={card.label}>
            <Card elevation={1} sx={{ p: 2, textAlign: "center", borderTop: `4px solid ${card.color}`, "&:hover": { boxShadow: 3 } }}>
              <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
                <Box sx={{ color: card.color, mb: 1 }}>{card.icon}</Box>
                <Typography variant="h4" sx={{ fontWeight: 700, mb: 1, color: card.color }}>
                  {card.value}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {card.label}
              ...
              {stats?.upcoming_tasks?.length > 0 ? (
                <Box sx={{ maxHeight: 300, overflow: "auto" }}>
                  {stats.upcoming_tasks.slice(0, 5).map((task, idx) => (
                    <Box
                      key={idx}
                      sx={{ p: 1.5, mb: 1, bgcolor: "#f5f5f5", borderRadius: 1, border: "1px solid #e0e0e0" }}
                    >
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {task.ngay_bat_dau}
                        {task.thoi_gian_bat_dau && ` - ${task.thoi_gian_bat_dau}`}
                      </Typography>
                      {task.ten_lo && (
