...
export default function DashboardHome() {
        ...
        const fetchStats = async() => {
            try {
                ...
                try {
                    ...
                    if (data.success) {
                        console.log('✅ API Success - Setting stats:', data.data);
                        setStats(data.data);
                        if (data.warning) {
                            console.warn('⚠️ Dashboard warning:', data.warning);
                        }
                        return;
                    } else {
                ...
                setStats({
                    total_users: 11, // Based on the image showing 11 users
                    total_lots: 0,
                    total_plans: 0,
                    today_tasks: 0,
    ...
    if (loading) {
        return ( <
            Box sx = {
                { display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px' } } >
            <
            CircularProgress / >
            <
            Typography sx = {
                { ml: 2 } } > Đang tải dữ liệu... < /Typography> <
            /Box>
        );
    }
    ...
    return ( <
        ...
        sx = {
            { fontWeight: 700, mb: 0.5 } } > Xin chào, Quản trị viên! < /Typography> <
        Typography variant = "body2"
        color = "text.secondary" > Theo dõi hoạt động nông trại, lô trồng và kế hoạch sản xuất tại đây. < /Typography> <
        /div> <
        Box sx = {
        ...
        Paper elevation = { 0 }
        sx = {
            { bgcolor: '#e3f2fd', p: 2, borderRadius: 2, mb: 2 } } >
        <
        Typography sx = {
            { fontWeight: 600, color: '#0d47a1' } } > Tổng quan hệ thống < /Typography> <
        /Paper>
        ...
        sx = {
            ...
            statCards.map((card, idx) => ( <
                ...
                <
                Box sx = {
                    { color: card.color, mb: 1 } } > { card.icon } <
                /Box> <
                Typography variant = "h4"
                sx = {
                    { fontWeight: 700, mb: 1, color: card.color } } > { card.value } <
                /Typography> <
                Typography variant = "body2"
                color = "text.secondary" > { card.label } <
                /Typography> <
                /CardContent> <
                /Card> <
                /Grid>
        ...
        { /* Nội dung dashboard */ } <
        Grid container spacing = { 2 } > { /* Biểu đồ trạng thái lô */ } <
        Grid item xs = { 12 }
        md = { 6 } >
        <
        Card elevation = { 1 } >
            ...
            stats ? .upcoming_tasks ? .length > 0 ? ( <
                Box sx = {
                    ...
                    stats.upcoming_tasks.slice(0, 5).map((task, idx) => ( <
                        ...
                        sx = {
                            { fontWeight: 600 } } > { task.ten_cong_viec } <
                        /Typography> <
                        Typography variant = "caption"
                        color = "text.secondary" > { task.ngay_bat_dau } { task.thoi_gian_bat_dau && `- ${task.thoi_gian_bat_dau}` } <
                        /Typography> {
                            task.ten_lo && ( <
                                Typography variant = "caption"
                                display = "block"
                                color = "text.secondary" >
                                Lô: { task.ten_lo }({ task.vi_tri }) <
                                /Typography>
                            )
                        } <
                        /Box>
