...
export default function CareMonitoring() {
    ...
    return ( <
        ...
        sx = {
            { fontWeight: 700, mb: 2 } } > Chăm sóc & theo dõi < /Typography>
        ...
        alignItems = {
            { md: "center" } } >
        <
        TextField label = "Chọn lô"
        select size = "small"
        value = { selectedLot }
        onChange = {
            (e) => setSelectedLot(e.target.value) }
        sx = {
            { minWidth: 240 } } >
        <
        MenuItem value = "" > Tất cả lô < /MenuItem> {
            lots.map(l => ( <
                MenuItem key = { l.ma_lo_trong || l.id }
                value = { l.ma_lo_trong || l.id } > { l.ma_lo_trong || l.id } <
                /MenuItem>
            ))
        } <
        /TextField> <
        ...
        /> <
        /Stack> <
        /Paper> { /* Alerts (AI/Weather/etc.) */ }

        {
            loading ? ( <
                    Box sx = {
                        { display: "flex", justifyContent: "center", mt: 6 } } > < CircularProgress / > < /Box>
                ) : ( <
                    Stack spacing = { 2 } > { /* Tasks / care logs */ } <
                    Paper elevation = { 0 }
                    sx = {
                        { p: 2, bgcolor: "white", border: "1px solid #eee" } } >
                    <
                    Stack direction = "row"
                    spacing = { 1 }
                    alignItems = "center"
                    ...
                    color = "text.secondary" > ({ lotTasks.length }
                        mục) < /Typography> <
                    /Stack> <
                    Divider sx = {
                        { mb: 2 } }
                        ...
                        lotTasks.length === 0 ? ( <
                            ...
                            Stack spacing = { 1.5 } > {
                                lotTasks.map(t => ( <
                                    Box key = { t.id }
                                    sx = {
                                        { display: "grid", gridTemplateColumns: { xs: "1fr", md: "2fr 1fr 1fr 1fr" }, gap: 2 } } >
                                    <
                                    Box >
                                    <
                                    Typography sx = {
                                        { fontWeight: 600 } } > { t.ten_cong_viec } < /Typography> <
                                    Typography variant = "body2"
                                    color = "text.secondary" > { t.mo_ta || "" } < /Typography> <
                                    /Box> <
                                    Box >
                                    <
                                    Typography variant = "body2"
                                    color = "text.secondary" > Lô < /Typography> <
                                    Typography > { t.ma_lo_trong || "" } < /Typography> <
                                    /Box> <
                                    Box >
                                    <
                                    Typography variant = "body2"
                                    color = "text.secondary" > Thời gian < /Typography> <
                    ...
                    Typography variant = "body2"
                    color = "text.secondary" > ({ lotMaterials.length }
                        mục) < /Typography> <
                    /Stack> <
                    Divider sx = {
                        { mb: 2 } }
                        ...
                        lotMaterials.length === 0 ? ( <
                            ...
                            Stack spacing = { 1.5 } > {
                                lotMaterials.map((m, idx) => ( <
                                    ...
                                    <
                                    Typography sx = {
                                        { fontWeight: 600 } } > { m.ten_vat_tu || m.ma_vat_tu || "Vật tư" } < /Typography> <
                                    Typography variant = "body2"
                                    color = "text.secondary" > { m.ghi_chu || "" } < /Typography> <
                                    /Box> <
                                    Box >
                                    <
                                    Typography variant = "body2"
                                    ...
                                    <
                                    Typography variant = "body2"
                                    color = "text.secondary" > Số lượng < /Typography> <
                                    Typography > { m.so_luong || m.dinh_luong || "" } < /Typography> <
                                    /Box> <
                                    Box >
                                    <
                                    Typography variant = "body2"
                                    color = "text.secondary" > Ngày < /Typography> <
                    ...
                    sx = {
                        { mb: 1 } } > Đề xuất thủ công < /Typography> <
                    TextField multiline minRows = { 3 }
                    fullWidth placeholder = "Nhập đề xuất xử lý (ví dụ: tưới bổ sung 2 ngày tới do nắng nóng)"
                    value = { manualSuggestion }
                    onChange = {
                        (e) => setManualSuggestion(e.target.value) }
                    /> <
                    Stack direction = "row"
                    spacing = { 1 }
                    sx = {
                    ...
                    endIcon = { < SendIcon / > }
                    onClick = { sendSuggestion } > Gửi đề xuất < /Button> {
                        sendStatus && < Chip label = { sendStatus }
                        color = { sendStatus === "Gửi thành công" ? "success" : "default" }
                        />} <
                        /Stack> <
                        /Paper> <
                        /Stack>
