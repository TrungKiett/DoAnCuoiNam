...
export default function UserManagement() {
    ...
    return ( <
        ...
        sx = {
            { fontWeight: 700 } } > Quản lý tài khoản < /Typography> <
        Box sx = {
        ...
        onClick = {
            () => setOpenAdd(true) } > +Thêm tài khoản < /Button> <
        /Box> <
        /Box>
        ...
        TableBody > {
            ...
            error && !loading && ( <
                ...
                TableCell colSpan = { 8 }
                sx = {
                    { color: 'error.main' } } > Lỗi: { error } < /TableCell> <
                /TableRow>
            )
        } {
            ...
            rows.map((u) => ( <
                ...
                color = "primary"
                onClick = {
                    () => { setEditForm({ id: u.id, username: u.username, password: '', full_name: u.fullName, phone: u.phone, role: u.role || 'Nông dân' });
                        setOpenEdit(true); } } > < EditIcon fontSize = "small" / > < /IconButton> <
                IconButton size = "small"
                color = "error"
                onClick = {
                    async() => {
                        ...
                        try {
                            ...
                            const mapped = (data || []).map((uu) => ({
                                ...
                                phone: uu.phone,
                                role: uu.role || 'Nông dân',
                                status: uu.status || 'Hoạt động',
        ...
        fullWidth / >
        <
        TextField select label = "Quyền truy cập"
        value = { form.role }
        ...
        fullWidth / >
        <
        TextField label = "Mật khẩu (để trống nếu giữ nguyên)"
        type = "password"
