...
export default function WorkerManagement() {
        ...
        return farmers
            .map(worker => {
                ...
                return {
                    ...
                    status,
                    availableHours,
                    priority: status.status === 'free' ? 3 : status.status === 'partial' ? 2 : 0
                };
            })
    ...
    return ( <
            ...
            <
            Grid container spacing = { 3 } > { /* 1. TỔNG QUAN SẴNG SÀNG THEO NGÀY */ } <
            Grid item xs = { 12 }
            lg = { 4 } >
            <
            Paper sx = {
            ...
            sx = {
                { mb: 2, display: 'flex', alignItems: 'center' } } > 📅 < Box sx = {
                { ml: 1 } } > Tổng quan Sẵn sàng Theo Ngày < /Box> <
            /Typography>

            { /* Date Picker */ } <
            TextField label = "Chọn ngày"
            type = "date"
            value = { selectedDate }
            onChange = {
            ...
            <
            Card sx = {
                { textAlign: 'center', bgcolor: 'success.light', color: 'white' } } >
            <
            CardContent sx = {
                { py: 2 } } >
            <
            ...
            <
            Card sx = {
                { textAlign: 'center', bgcolor: 'warning.light', color: 'white' } } >
            <
            CardContent sx = {
                { py: 2 } } >
            <
            ...
            <
            Card sx = {
                { textAlign: 'center', bgcolor: 'error.light', color: 'white' } } >
            <
            CardContent sx = {
                { py: 2 } } >
            <
            ...
            <
            Card sx = {
                { textAlign: 'center', bgcolor: 'grey.600', color: 'white' } } >
            <
            CardContent sx = {
                { py: 2 } } >
            <
            ...
            List dense > {
                filteredWorkers.map((worker) => {
                    ...
                    return ( <
                        ...
                        Avatar sx = {
                            {
                                bgcolor: status.color === 'success' ? 'success.main' : status.color === 'warning' ? 'warning.main' : status.color === 'error' ? 'error.main' : 'grey.500',
                                width: 32,
                                height: 32
                            }
                        } > {
                        ...
                        /Avatar> <
                        /ListItemAvatar> <
                        ListItemText primary = { worker.full_name || `ND-${worker.id}` }
                        secondary = { <
                            Box sx = {
                            ...
                            >
                            <
                            Typography variant = "caption" > { status.hours }
                            h / { status.maxHours }
                            h <
                            /Typography> <
                            /Box>
            ...
            sx = {
                { mb: 2, display: 'flex', alignItems: 'center' } } > 📊 < Box sx = {
                { ml: 1 } } > Biểu đồ Tải công việc & Sẵn sàng < /Box> <
            /Typography>
            ...
            Box sx = {
                ...
                farmers.slice(0, 20).map((worker) => { // Show first 20 for demo
                    ...
                    return ( <
                        ...
                        sx = {
                            { mb: 0.5, fontWeight: 'bold' } } > { worker.full_name || `ND-${worker.id}` } <
                        /Typography> <
                        Box sx = {
                            ...
                            dates.map((date) => {
                                ...
                                return ( <
                                    ...
                                    title = { <
                                        ...
                                        Typography variant = "body2" > { status.hours }
                                        h / { status.maxHours }
                                        h < /Typography> {
                                            status.tasks && status.tasks.length > 0 && ( <
                                                Box sx = {
                                                    ...
                                                    status.tasks.map((task, idx) => ( <
                                                        Typography key = { idx }
                                                        variant = "caption"
                                                        display = "block" > •{ task.ten_cong_viec }({ task.thoi_gian_bat_dau } - { task.thoi_gian_ket_thuc }) <
                                                        /Typography>
                                                    ))
                                                } <
                                                /Box>
                                    ...
                                    Box sx = {
                                        {
                                            width: 20,
                                            height: 20,
                                            bgcolor: status.status === 'free' ? 'success.light' : status.status === 'partial' ? 'warning.light' : status.status === 'busy' ? 'error.light' : 'grey.400',
            ...
            sx = {
                { mb: 2, display: 'flex', alignItems: 'center' } } > 🔍 < Box sx = {
                { ml: 1 } } > Công cụ Phân công Nhanh < /Box> <
            /Typography>
            ...
            Typography variant = "subtitle2"
            sx = {
                { mb: 2, fontWeight: 'bold' } } > 🏆TOP Nhân công sẵn sàng <
            /Typography>
            ...
            <
            Box sx = {
                { mt: 2, display: 'grid', gap: 2 } } >
            <
            TextField label = "Tên nhiệm vụ"
            value = { quickTask.title }
            onChange = {
                (e) => setQuickTask({...quickTask, title: e.target.value }) }
            fullWidth /
            >

            <
            TextField label = "Ngày thực hiện"
            type = "date"
            value = { quickTask.date }
            onChange = {
                (e) => setQuickTask({...quickTask, date: e.target.value }) }
            InputLabelProps = {
                { shrink: true } }
            fullWidth /
            >
            ...
            Select value = { quickTask.timeSlot }
            label = "Ca làm việc"
            onChange = {
                (e) => setQuickTask({...quickTask, timeSlot: e.target.value }) } >
            <
            MenuItem value = "morning" > Ca sáng(7 h - 11 h) < /MenuItem> <
            MenuItem value = "afternoon" > Ca chiều(13 h - 17 h) < /MenuItem> <
            MenuItem value = "full" > Cả ngày(7 h - 17 h) < /MenuItem> <
            /Select> <
            /FormControl>
            ...
            Select value = { quickTask.location }
            label = "Địa điểm (Lô)"
            onChange = {
                (e) => setQuickTask({...quickTask, location: e.target.value }) } >
            {
                getAllLots().map((lot) => ( <
                    MenuItem key = { lot.ma_lo_trong }
                    value = { lot.ma_lo_trong } >
            ...
            <
            List dense sx = {
                { bgcolor: 'grey.50', borderRadius: 1, maxHeight: 200, overflow: 'auto' } } > {
                getSuggestedWorkers().map((worker, index) => {
                        ...
                        return ( <
                                ...
                                secondaryAction = { <
                                    ...
                                    onClick = {
                                        () => isAssigned ? removeAssignedWorker(worker.id) : handleQuickAssign(worker.id) }
                                    disabled = {!quickTask.title || !quickTask.timeSlot || !quickTask.location } >
                                    { isAssigned ? "Đã chọn" : "Phân công" } <
                                    /Button>
                                } >
                                <
                                ListItemText primary = { `${index + 1}. ${worker.full_name || `ND-${worker.id}`}`}
