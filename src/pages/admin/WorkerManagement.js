...
export default function WorkerManagement() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [farmersRes, tasksRes, leaveRes, lotsRes, urgentTasksRes] =
        await Promise.all([
          fetchFarmers(),
          listTasks(),
    ...
    return farmers
      .map((worker) => {
        ...
        return {
          ...
          availableHours,
          priority:
            status.status === "free" ? 3 : status.status === "partial" ? 2 : 0,
        };
      })
      .filter((worker) => worker.availableHours >= 4) // Minimum 4 hours available
      .sort(
        (a, b) => b.priority - a.priority || b.availableHours - a.availableHours
      )
      .slice(0, 5);
  };
  ...
  const getUrgentTasks = () => {
    return (Array.isArray(urgentTasks) ? urgentTasks : []).sort(
  ...
  const createUrgentTasks = async () => {
    ...
    try {
      ...
      const assignedWorkerNames = quickTask.assignedWorkers
        .map((workerId) => {
      ...
      if (result.success) {
        ...
        alert(
          "Không thể tạo nhiệm vụ khẩn cấp: " +
  ...
  return (
    <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ maxHeight: 400, overflow: "auto" }}>
              ...
              <List dense>
                ...
                {filteredWorkers.map((worker) => {
                  const status = getWorkerStatusForDate(
                    worker.id,
                    selectedDate
                  );
                  return (
                  ...
                  <Box key={worker.id} sx={{ mb: 2 }}>
                    ...
                    <Box sx={{ display: "flex", gap: 0.5 }}>
                      ...
                      {dates.map((date) => {
                        ...
                        const percentage =
                          (status.hours / status.maxHours) * 100;
        ...
        <DialogContent>
          <Box sx={{ mt: 2, display: "grid", gap: 2 }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setQuickTask({ ...quickTask, date: e.target.value })
              }
              InputLabelProps={{ shrink: true }}
              fullWidth
            />
            <FormControl fullWidth>
              ...
              <Select
                ...
                label="Ca làm việc"
                onChange={(e) =>
                  setQuickTask({ ...quickTask, timeSlot: e.target.value })
                }
              >
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                label="Địa điểm (Lô)"
                onChange={(e) =>
                  setQuickTask({ ...quickTask, location: e.target.value })
                }
              >
                {getAllLots().map((lot) => (
            ...
            >
              {" "}
              {getSuggestedWorkers().map((worker, index) => {
                const isAssigned = quickTask.assignedWorkers.includes(
                  worker.id
                );
                return (
                  <ListItem
                    ...
                    secondaryAction={
                      <Button
                        ...
                        onClick={() =>
                          isAssigned
                            ? removeAssignedWorker(worker.id)
                            : handleQuickAssign(worker.id)
                        }
                        disabled={
                          !quickTask.title ||
                          !quickTask.timeSlot ||
        ...
        {getUrgentTasks().length === 0 ? (
          ...
          <Grid container spacing={2}>
            {getUrgentTasks().map((task, index) => {
              const assignedWorkers = task.nguoi_tham_gia
                ? task.nguoi_tham_gia.split(",")
                : [];
              const workerNames = assignedWorkers
                .map((workerId) => {
                  ...
                  const worker = farmers.find(
                    (f) => f.id == actualId || f.id === actualId
                  );
                  ...
                  >
                    <CardContent>
                      ...
                      >
                        ...
                        <Chip label="Khẩn cấp" color="error" size="small" />
                      </Box>
                      ...
                      >
                        📅 Ngày:{" "}
                        {task.ngay
                          ? new Date(task.ngay).toLocaleDateString("vi-VN")
                          : "Chưa xác định"}
                      </Typography>
                      ...
                      <Typography
                        ...
                        sx={{ mb: 1 }}
                      >
                        👥 Người tham gia ({assignedWorkers.length}):{" "}
                        {workerNames}
                      </Typography>
                      ...
                      >
                        ...
                        >
                          <Chip
                            label={
                              task.trang_thai === "chua_bat_dau"
                                ? "Chưa bắt đầu"
                                : task.trang_thai === "dang_thuc_hien"
                                  ? "Đang thực hiện"
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ...
            {selectedUrgentTask?.ten_nhiem_vu}"?
          </Typography>
        ...
        <DialogActions>
          <Button onClick={() => setDeleteUrgentDialog(false)}>Hủy</Button>
          <Button
            variant="contained"
            color="error"
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setEditUrgentDialog(false)}>Đóng</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
