...
export default function PayrollReports() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const weekNow = getWeekOfMonthIndex(
        today.getFullYear(),
        today.getMonth() + 1,
        today.getDate()
      );
  ...
  const buildCompletedTasksForWorker = (workerId) => {
    ...
    const isCompleted = (status) =>
      status === "hoan_thanh" || status === "da_hoan_thanh";
    ...
    const filtered = (tasks || []).filter((t) => {
      ...
      const assigned = String(t.ma_nguoi_dung || "")
        ...
        .map((s) => s.trim());
      return (
        assigned.includes(String(workerId)) || assigned.includes(workerCode)
      );
    });
  ...
  const loadPayrollData = async () => {
    try {
      ...
      console.log(
        "Loading payroll for date range:",
        startDateStr,
        "to",
        endDateStr
      );
      ...
      const response = await fetchPayrollData(
        startDateStr,
        endDateStr,
        payrollPeriod === "weekly" ? selectedWeek : undefined,
        selectedYear
      );
  ...
  const calculateWorkerHours = (workerId, startDate, endDate) => {
    const workerTasks = tasks.filter((task) => {
      ...
      const assignedWorkers = String(task.ma_nguoi_dung)
        .split(",")
        .map((id) => id.trim());
      const taskDate = new Date(task.ngay_bat_dau);
      return (
        assignedWorkers.includes(String(workerId)) &&
        taskDate >= startDate &&
        taskDate <= endDate
      );
    });
  ...
  const getPayrollSummary = () => {
    ...
    const totalCost = processedData.reduce(
      (sum, worker) => sum + worker.totalPay,
      0
    );
  ...
  const handleSelectAll = (event) => {
    ...
    const selectable = processedPayrollData.filter(
      (w) => w.status === "pending" && (w.totalHours || 0) > 0
    );

    if (isChecked) {
      setSelectedWorkers(selectable.map((w) => w.id));
    } else {
      setSelectedWorkers([]);
    }
  };
  ...
  const handleApproveSelected = () => {
    ...
    const processedData = getPayrollData();
    const zeroHourSelected = processedData.some(
      (w) => selectedWorkers.includes(w.id) && (w.totalHours || 0) === 0
    );
    if (zeroHourSelected) {
    ...
    if (
      window.confirm(
        `Xác nhận duyệt lương cho ${selectedWorkers.length} nhân công được chọn?`
      )
    ) {
      const processedData = getPayrollData();
      const { startDate, endDate } = getPayrollPeriodDates();
      ...
      const periodName = `Chi tiết Bảng lương - ${
        payrollPeriod === "weekly" ? `Tuần ${week}` : `Tháng ${selectedMonth}`
      }/${year}`;
      ...
      Promise.all(
        ...
        .catch((err) => {
          ...
          alert("Không thể duyệt hàng loạt: " + err.message);
        });
    }
  };
  ...
  const handleRejectSelected = () => {
    ...
    if (
      window.confirm(
        `Xác nhận từ chối lương cho ${selectedWorkers.length} nhân công được chọn?`
      )
    ) {
      const processedData = getPayrollData();
      const { startDate, endDate } = getPayrollPeriodDates();
      ...
      Promise.all(
        processedData
          ...
          .map((worker) =>
            upsertPayrollRecord({
              ...
              total_hours: worker.totalHours,
              hourly_rate: worker.hourlyRate,
              status: "pending", // hoặc "rejected" tùy yêu cầu backend
              week,
              year,
              period_name: periodName,
            })
        ...
        .catch((err) => {
          console.error("Error rejecting payrolls:", err);
          alert("Không thể từ chối hàng loạt: " + err.message);
        });
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
                ...
                <TableHead>
                  <TableRow>
                    <TableCell padding="checkbox">
                      <Checkbox
                        checked={
                          processedPayrollData.filter(
                            (w) =>
                              w.status === "pending" && (w.totalHours || 0) > 0
                          ).length > 0 &&
                    ...
                    <TableRow key={worker.id} hover>
                      <TableCell padding="checkbox">
                        <Checkbox
                          ...
                          disabled={
                            (["pending", "Chờ duyệt"].includes(worker.status)
                              ? false
                              : true) || (worker.totalHours || 0) === 0
                          }
                        />
                      </TableCell>
                      ...
                      <TableCell>
                        <Tooltip title="Xem chi tiết chấm công">
                          <IconButton
                            size="small"
                            onClick={() => {
                              const built = buildCompletedTasksForWorker(
                                worker.id
                              );
              ...
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={3}>
                  <Typography variant="body1">
                    Tổng giờ:{" "}
                    <strong>{summary.totalHours.toLocaleString()} giờ</strong>
                  </Typography>
                </Grid>
                <Grid item xs={3}>
                  <Typography variant="body1">
                ...
                <Grid item xs={2}>
                  <Typography variant="body1">
                    TB/người:{" "}
                    <strong>
                      {(summary.totalCost / summary.workerCount).toLocaleString(
                        "vi-VN"
                      )}{" "}
                      ₫
                    </strong>
      ...
      >
        <DialogTitle>
          📋 Chi tiết chấm công -{" "}
          {selectedWorker?.full_name || `ND-${selectedWorker?.id}`}
        </DialogTitle>
        <DialogContent>
          {selectedWorker && (
            <Box sx={{ mt: 2 }}>
              ...
              <Box sx={{ mt: 3 }}>
                ...
                <Grid container spacing={1}>
                  {Object.entries(selectedWorker.dailyHours || {}).map(
                    ([date, hours]) => (
                      <Grid item xs={3} key={date}>
                        <Card
                          sx={{
                            p: 1,
                            textAlign: "center",
                            bgcolor: hours > 8 ? "warning.light" : "grey.100",
                          }}
                        >
